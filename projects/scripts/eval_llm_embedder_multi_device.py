...
if __name__ == "__main__":
    ...
    n_gpu = len(devices)
    # cut corpus, querie into n_gpu parts
    corpus_parts = np.array_split(corpus, n_gpu)
    queries_parts = np.array_split(queries, n_gpu)
    examples_prefix_list_parts = np.array_split(examples_prefix_list, n_gpu)
    
    doc_results = [None] * n_gpu  # 预分配
    doc_results_lock = Lock()

    def run_inference_doc(model, tokenizer, corpus_part, device, index):
        result = inference_doc(corpus_part, tokenizer, model, args.doc_max_len, args.batch_size, device)
        with doc_results_lock:
            doc_results[index] = result

    query_results = [None] * n_gpu  # 预分配
    query_results_lock = Lock()

    def run_inference_query(model, tokenizer, queries_part, examples_prefix_list_part, device, index):
        result = inference_query_examples_list(queries_part, args.query_max_len, examples_prefix_list_part, tokenizer, model, args.batch_size, device)
        with query_results_lock:
            query_results[index] = result
    
    threads = []
    for index, device in enumerate(devices):
        thread = Thread(target=run_inference_doc, args=(models[index][0], models[index][1], corpus_parts[index].tolist(), device, index))
    ...
    threads = []
    for index, device in enumerate(devices):
        thread = Thread(target=run_inference_query, args=(models[index][0], models[index][1], queries_parts[index].tolist(), examples_prefix_list_parts[index].tolist(), device, index))
        threads.append(thread)
    ...
    print(f"DEBUG: len doc_results: {len(doc_results)}, len query_results: {len(query_results)}")
    print(f"DEBUG: doc_results[0].shape: {doc_results[0].shape}, query_results[0].shape: {query_results[0].shape}")
    
    # 合并结果前检查
    if any(x is None for x in doc_results) or any(x is None for x in query_results):
        raise RuntimeError("Some GPU tasks failed to complete")

    doc_embeddings = np.concatenate(doc_results, axis=0)
    query_embeddings = np.concatenate(query_results, axis=0)
    print(f"DEBUG: doc_embeddings.shape: {doc_embeddings.shape}, query_embeddings.shape: {query_embeddings.shape}")
